    ...
    , transpilationSuccess(false)
{
    ...
    // Configurar ruta del transpilador
    transpiladorPath = QDir::currentPath() + "/../transpilador";
...
void MainWindow::executeTranspilation()
{
    ...
    transpilationProcess->start("./transpilador", QStringList() << currentFile);
}
...
void MainWindow::processFinished(int exitCode, QProcess::ExitStatus status)
{
    if (status == QProcess::NormalExit && exitCode == 0) {
        ...
        compileProcess.start("g++", QStringList() << "salida.cpp" << "-o" << "simulador");
        ...
        if (compileProcess.exitCode() == 0) {
            ...
            simulatorProcess->start("./simulador", QStringList());
...
void MainWindow::validateSyntaxRealTime()
{
    ...
    validationProcess->start("./transpilador", QStringList() << tempFile);
}
